...
dataset = load_dataset("eduagarcia/enem_challenge", split="train")
...
def get_prompt(example, n_shots=args.n_shots, n_experiments=args.n_experiments):
    for i in range(n_experiments):
        ...
        for shot in shots:
            shot_alternatives = [f"({chr(65 + asc_letter)}): {shot['choices']['text'][asc_letter]}" for asc_letter in range(5)]
            shot_alternatives = "\n".join(shot_alternatives)
            
            user_shot_message = f"""Pergunta:\n{shot["question"]}.\n\nLeia as alternativas abaixo e responda corretamente a pergunta:\n\n{shot_alternatives}"""
            assistant_shot_message = f"""Lendo as alternativas (A), (B), (C), (D) (E), a alternativa que responde corretamente a pergunta é a letra ({shot['answerKey']})"""
            
            chat.append({"role": "user", "content": user_shot_message})
            chat.append({"role": "assistant", "content": assistant_shot_message})
        
        
        user_alternatives = [f"({chr(65 + asc_letter)}): {example['choices']['text'][asc_letter]}" for asc_letter in range(5)]
        user_alternatives = "\n".join(user_alternatives)
        user_message = f"""Pergunta:\n{example["question"]}.\n\nLeia as alternativas abaixo e responda corretamente a pergunta:\n\n{user_alternatives}"""
        chat.append({"role": "user", "content": user_message})
        chat.append({"role": "assistant", "content": "Lendo as alternativas (A), (B), (C), (D), (E), a alternativa que responde corretamente a pergunta é a letra ("})
...
dataset_name = f'enem_{args.n_shots}shot_{args.n_experiments}exp'
