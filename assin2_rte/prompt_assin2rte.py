...
dataset = load_dataset("eduagarcia/portuguese_benchmark", 'assin2-rte', split="test")
...
def get_prompt(example, n_shots=args.n_shots, n_experiments=args.n_experiments):
    for i in range(n_experiments):
        ...
        for shot in shots:
            user_shot_message = f"""Premissa: {shot["sentence1"]}.\nHipótese: {shot["sentence2"]}.\n\nCom base na nossa premissa, a hipótese acima é verdadeira ou falsa? Responda com Sim ou Não."""
            assistant_shot_message = "Resposta: Sim, com base na nossa premissa, a hipótese acima é verdadeira." if shot['label'] == 1 else "Resposta: Não, com base na nossa premissa, a hipótese acima não é verdadeira."
            
            chat.append({"role": "user", "content": user_shot_message})
            chat.append({"role": "assistant", "content": assistant_shot_message})
        
        user_message = f"""Premissa: {example["sentence1"]}.\nHipótese: {example["sentence2"]}.\n\nCom base na nossa premissa, a hipótese acima é verdadeira ou falsa? Responda com Sim ou Não."""
...
dataset_name = f'assin2rte_{args.n_shots}shot_{args.n_experiments}exp'
